...
from suda_ir.ir.fuzzy import fuzzy_field_bonus
from suda_ir.ir.index import FIELD_WEIGHTS, SearchResult
from suda_ir.ir.query_expansion import expand_query, weighted_query_terms
    ...
    def search(
        ...
        *,
        ...
        for doc_index, doc in enumerate(self.docs):
            ...
            if use_fuzzy and 2 <= len(query.strip()) <= 6:
                score += fuzzy_field_bonus(query, doc, ["name"])

            if doc.name and query.strip() == doc.name:
                score += 100.0

            if score > 0:
                matched = self._matched_terms(doc_index, terms)
